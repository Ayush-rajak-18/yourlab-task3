  ...
  return (
    <div>
       <div>
        ...
        <div onClick={()=>setSelectVal(first) || setToggle(false)} className="p-[0.2rem] m-0 box-content rounded-lg hover:bg-[#5F9EA0]">{first}</div>
        <div onClick={()=>setSelectVal(second) || setToggle(false)} className="p-[0.2rem] m-0 box-content rounded-lg hover:bg-[#00FFFF]">{second}</div>
        </div>
        }
       </div>
    </div>
